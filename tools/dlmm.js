...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    if (tracked) {
      ...
      // Reject obviously absurd closed PnL as unsettled noise (unless stop-loss confirms it).
      // Meteora sometimes briefly reports -90%+ while the record is settling.
          ...
          if (res.ok) {
            ...
            if (posEntry) {
              ...
              if (shouldRejectClosedPnl(nextPnlPct, reason || tracked?.close_reason)) {
                log("close_warn", `Rejected unsettled closed PnL for ${position_address.slice(0, 8)} on attempt ${attempt + 1}: ${nextPnlPct.toFixed(2)}% — retrying`);
              } else {
                pnlUsd        = nextPnlUsd;
                pnlPct        = nextPnlPct;
